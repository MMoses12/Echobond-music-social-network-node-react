...
function CountrySelection (user) {
    const changeCountry = (event) => {
        user.setCountry(event.target.value)
    ...
    return (
        <div>
            <select id="country" value={user.country} onChange={changeCountry} name="country" className="bg-transparent rounded-sm border-2 border-gray-200 hover:border-purple-300 w-full h-12 pl-3 text-white">
                {countries.map(country => (
                    <option key={country} value={country} className="bg-black-200"> {country} </option>
                ))}
            </select>
        </div>
    );
